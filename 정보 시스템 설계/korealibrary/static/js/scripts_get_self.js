document.addEventListener('DOMContentLoaded', function() {
    ...
    memberForm.addEventListener('submit', function(event) {
        event.preventDefault(); // form은 버튼을 들고 있을 수 있음. form의 액션값중 하나가 submit.되면 밑에 동작. 페이지 이동x
        const memberId = document.getElementById('memberId').value.trim();
        const memberName = document.getElementById('memberName').value.trim(); // input으로 들어온 값을 변수화(위치값->변수값에 넣음)
