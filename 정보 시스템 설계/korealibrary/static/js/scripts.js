document.addEventListener('DOMContentLoaded', function() {
    ...
    function approveLoan(book_code, book_location) {
        const dataUrl = `http://127.0.0.1:7000/approve/?book_code=${encodeURIComponent(book_code)}&book_location=${encodeURIComponent(book_location)}`;
        ...
        fetch(dataUrl)
            ...
            .then(data => {
                ...
                if (!approvalBody) {
                    console.error('approvalBody가 존재하지 않습니다.');
                    return;
                }
    ...
    function openLocationModal(approval_status) {
        ...
        if (approval_status === 'N') {
         ...
         saveButton.onclick = function() {
                ...
                // 실제 DB 업데이트를 하지 않으므로, 단순히 알림만 표시합니다.
                $(modal).modal('hide');
        };
